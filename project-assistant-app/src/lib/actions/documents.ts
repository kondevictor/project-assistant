...
export async function generateDocument(input: DocumentRequestInput) {
  const validated = DocumentRequestSchema.parse(input);
  ...
  const template = TEMPLATES[validated.templateType];
  if (!template) {
    throw new Error("Invalid template type");
  }

  // Prepare template data with project info
  const templateData = {
...
export async function initializeTemplates() {
  ...
  const templates = [
    { name: "NCNDA", type: "ncnda", description: "Non-Compete Non-Disclosure Agreement", content: "" },
